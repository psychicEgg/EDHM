...
> **Developer testing preview only** on branch **`shader-files-updated`**. This is **not** an official EDHM release. The unpacked ShaderFixes / ini payload and `EDHM_Odyssey_v22.02.zip` here are for rematch validation and tester feedback ahead of a real release.

Preview target: Elite Dangerous v4.4.1.0 *(Rhino / Surface Mining)*. Changelog for files touched on this branch: [root README](../README.md#preview-changelog---files-touched-on-this-branch) and [preview notes](EDHM%20Patch%20Notes%20-%20FDev%20U22.x.md).
